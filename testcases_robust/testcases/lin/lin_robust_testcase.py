...
from testcases_canlin.lin.lin_module import lin_initialization, lin_deinitialization
...
def test_TG1_TC1_LINBusShortCircuitRecoveryTest():
    ...
    try:
        ...
        for fault in lin_faults:
            ...
            try:
                kind = fault.replace('LIN_short', 'SHORT')
                ctx.bob_ctrl.set_fault(f"LIN{lin_channel}", kind, enable=True)
                time.sleep(5)
                ctx.bob_ctrl.set_fault(f"LIN{lin_channel}", kind, enable=False)
                time.sleep(3)
...
def test_TG1_TC4_LINMasterLossRecoveryTest():
    """LIN主节点丢失鲁棒性测试 - 模拟LIN主节点丢失，验证DUT CAN通信维持和LIN恢复"""
    try:
        rVnormal = P.LINInfo.Vnormal if hasattr(P.LINInfo, 'Vnormal') else 12.0
        rTstable_s = P.LINInfo.Tstable_s if hasattr(P.LINInfo, 'Tstable_s') else 5.0
        lin_channel = DEFAULT_LIN_CHANNEL
        ...
        TestLog("INFO", "Step2", "通过给LIN总线断电模拟主节点丢失，观察DUT响应")
        ctx.lin.clear_messages()

        # 模拟主节点丢失：断开LIN终端
        ctx.bob_ctrl.set_fault(f"LIN{lin_channel}", 'OPEN', enable=True)
        TestLog("INFO", "Step2", "LIN主节点丢失（总线开路），等待15秒")
        time.sleep(15)

        TestLog("INFO", "Step3", "恢复LIN主节点（恢复总线连接），验证LIN通信恢复")
        ctx.bob_ctrl.set_fault(f"LIN{lin_channel}", 'OPEN', enable=False)
        time.sleep(5)
        ...
        if lin_msgs > 0:
            TestLog("PASS", "Step3",
                    f"期望结果：LIN主节点恢复后通信正常。实际结果：收到{lin_msgs}条LIN报文")
        else:
            TestLog("WARNING", "Step3",
                    "期望结果：LIN主节点恢复后通信正常。实际结果：未收到LIN报文")

        TestLog("INFO", "Step4", "验证CAN侧通信在LIN故障期间是否维持正常")
        ...
        can_msgs = len(ctx.can.messages)
        can_errs = ctx.can.get_info('gErrorFrameCount') or 0
        if can_msgs > 0 and can_errs == 0:
            TestLog("PASS", "Step4",
                    f"期望结果：LIN故障期间CAN通信维持正常。实际结果：{can_msgs}条CAN报文, 0个错误帧")
        else:
            TestLog("WARNING", "Step4",
                    f"CAN通信状态: 报文={can_msgs}, 错误帧={can_errs}")

        TestLog("INFO", "LIN主节点丢失鲁棒性测试", "测试完成")

    except Exception as e:
        TestLog("FAIL", "LIN主节点丢失鲁棒性测试", f"测试执行出错: {e}")
        import traceback
        TestLog("DEBUG", "LIN主节点丢失鲁棒性测试", f"详细错误: {traceback.format_exc()}")
