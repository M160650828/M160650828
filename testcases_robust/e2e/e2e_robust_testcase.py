...
from testcases_canlin.e2e.e2e_module import (
...
def test_TG1_TC4_E2EDualFaultTest():
    """E2E双故障鲁棒性测试 - 同时注入E2E CRC错误与CAN通信故障"""
    try:
        rVnormal = P.CANInfo.Vnormal
        rTstable_s = P.CANInfo.Tstable_s
        can_channel = DEFAULT_CAN_CHANNELS[0] if DEFAULT_CAN_CHANNELS else 1
        ...
        ret = check_can_communication_state(wait_time=3)
        if ret != 0:
            TestLog("FAIL", "Step3", "双故障清除后通信未恢复")
            return

        TestLog("INFO", "Step3", "通信恢复，进行E2E CRC和Counter校验")
        verify_can_crc(profile_1a=True)
        verify_can_counter(profile_1a=True)

        TestLog("PASS", "Step3",
                "期望结果：双故障清除后E2E通信恢复正常。实际结果：CRC和Counter校验通过")
        TestLog("INFO", "E2E双故障鲁棒性测试", "测试完成")

    except Exception as e:
        TestLog("FAIL", "E2E双故障鲁棒性测试", f"测试执行出错: {e}")
        import traceback
        TestLog("DEBUG", "E2E双故障鲁棒性测试", f"详细错误: {traceback.format_exc()}")
