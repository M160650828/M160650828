...
from testcases_canlin.dtc.can_comm import can_power_setup_and_communication_check, can_initialization, can_deinitialization
from testcases_canlin.dtc.dtc_can_utils import (
...
def test_TG1_TC4_DTCOverflowStressTest():
    """DTC溢出压力鲁棒性测试"""
    node = None
    try:
        rVnormal = P.CANInfo.Vnormal
        ...
        TestLog("INFO", "Step2", f"快速反复注入并恢复故障{max(P.CANInfo.Tcount, 10)}次，模拟DTC溢出条件")
        burst_count = max(P.CANInfo.Tcount, 10)
        for i in range(1, burst_count + 1):
            TestLog("INFO", "Step2", f"第{i}/{burst_count}次故障注入-恢复循环")
            inject_fault_and_read_global_snapshot(node, fault_type, step=f"Step2-注入{i}")
            time.sleep(0.5)
            recover_fault_and_read_global_snapshot(node, fault_type, step=f"Step2-恢复{i}")
            time.sleep(0.5)

        TestLog("INFO", "Step3", "压力注入后读取全部DTC列表")
        success, resp = service_19_check(node, report_type=0x0A,
                                         expect_data=[0x59, 0x0A],
                                         expect_str="肯定响应(59 0A)",
                                         func_req=False)
        if success:
            dtc_list = get_dtc_list_from_19_resp(resp)
            dtc_count = len(dtc_list)
            TestLog("INFO", "Step3", f"溢出压力后DTC列表包含{dtc_count}条记录")

            if dtc_count > 0:
                check_dtc_list_status_bits(dtc_list, DTCStatusBit.CONFIRMED,
                                          log_prefix="Step3-溢出压力后DTC状态")
                TestLog("PASS", "Step3",
                        f"期望结果：溢出压力后DTC状态管理正常。"
                        f"实际结果：{dtc_count}条DTC记录, DTC管理未崩溃")
            else:
                TestLog("INFO", "Step3", "DTC列表为空，所有DTC可能已被正确处理")
                TestLog("PASS", "Step3", "期望结果：DTC管理未崩溃。实际结果：DTC列表正常返回")
        else:
            TestLog("FAIL", "Step3", "读取DTC列表失败")

        TestLog("INFO", "Step4", "清除所有DTC并验证清除操作可正常完成")
        clear_dtc(node)
        time.sleep(2)
        success2, resp2 = service_19_check(node, report_type=0x0A,
                                         expect_data=[0x59, 0x0A],
        ...
        if success2:
            dtc_list2 = get_dtc_list_from_19_resp(resp2)
            confirmed_after_clear = sum(
                1 for d in dtc_list2
                if (d['status'] & DTCStatusBit.CONFIRMED.value) != 0
            )
            if confirmed_after_clear == 0:
                TestLog("PASS", "Step4",
                        "期望结果：溢出压力后DTC可正常清除。实际结果：confirmed DTC数量为0")
            else:
                TestLog("WARNING", "Step4",
                        f"清除后仍有{confirmed_after_clear}个confirmed DTC")
        else:
            TestLog("FAIL", "Step4", "读取DTC列表失败")

        TestLog("INFO", "DTC溢出压力鲁棒性测试", "测试完成")

    except Exception as e:
        TestLog("FAIL", "DTC溢出压力鲁棒性测试", f"测试执行出错: {e}")
        import traceback
        TestLog("DEBUG", "DTC溢出压力鲁棒性测试", f"详细错误: {traceback.format_exc()}")
    finally:
        env_simulator.stop()
        if node:
            node.close()
