...
def ActivateDut(rSimulationActivate=1, rTstable=20, power_off: float = None):
    ...
    try:
        ...
        TestLog("DEBUG", "DUT模式", f"当前 DUT 模式: {dut_mode}")
        ...
        # Step1: 设置DUT供电电压
        TestLog("DEBUG", "", f"设置DUT供电电压为 {rVnormal:.2f}V")
        ...
        # Step4: 等待通信稳定并检查通信状态
        if rTstable != 0:
            TestLog("DEBUG", "", f"等待 {rTstable}s 以检查LIN通信状态,LDF:{len(_lin_slave_frame_info)}")
        ...
        TestLog("DEBUG", "", f"按 DUT 模式({dut_mode}) 启动仿真成功，通信正常")
        return 0
