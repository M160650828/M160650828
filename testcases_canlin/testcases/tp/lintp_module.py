...
def lin_tp_initialization(channel=None,sim_nadid = None,net_work =None,test_slave_flg=True,funcrequest_in_phyresponse_flg=False):
    ...
    try:
        TestLog("INFO", "LINTP测试初始化函数", "开始LINTP协议相关初始化")
        if sim_nadid == None:
...
def lintp_send_req_by_message(data:bytes,delay_0x3c:float,lin_channel=None,nadid=None,add_msg=None):
    ...
    if add_msg!=None and len(msgs)>1:
        msgs = msgs[0:1] + [add_msg] + msgs[1:]
