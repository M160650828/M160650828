...
def test_TG2_TC3_DataLinkLayer_MaximumFillBitMessageTest():
    """最大填充位报文测试"""
    try:
        # 测试参数
        ...
        # 创建CAN报文
        msg = canmsg_create(rMsgID, 8, data=0x3C, rtr=0, fdf=0, brs=0, ext=0)
...
def test_TG2_TC4_DataLinkLayer_ExtendFrameTest():
    """扩展帧测试"""
    try:
        # 测试参数
        rVnormal = P.CANInfo.Vnormal  # 电源正常电压
...
def test_TG2_TC5_DataLinkLayer_RemoteFrameTest():
    """远程帧测试"""
    try:
        # 测试参数
        rVnormal = P.CANInfo.Vnormal  # 电源正常电压
...
def test_TG2_TC6_DataLinkLayer_ErrorFrameTest():
    """错误帧测试"""
    try:
        # 测试参数
        rVnormal = P.CANInfo.Vnormal  # 电源正常电压
...
def test_TG2_TC13_DataLinkLayer_BusLoadMiddleRateMonitoringTest():
    ...
    try:
        ...
        # 创建CAN报文
        msg1 = canmsg_create(0x001, 8, data=0x3C, rtr=0, fdf=0, brs=0, ext=0)
        tids = []
        ...
        msg2 = canmsg_create(0x7FF, 8, data=0x3C, rtr=0, fdf=0, brs=0, ext=0)
        start_time = time.time()
        while round(sl_busstatis().get_can_stat_by_ch(can_channel)["busload"]["cur"] * 100, 2) < rBusloadNormal:
            if time.time() - start_time > 2 * 60:
                busload = sl_busstatis().get_can_stat_by_ch(can_channel)
...
def test_TG2_TC16_VoltageJumpRecoveryCommunicationDiagnosticStrategyTest():
    """
    电压跳变恢复后通信诊断策略测试
    """
    try:
...
def test_TG2_TC17_ColdStartRapidPowerCycleTest():
    """
    冷启动快速上下电测试
    """
    try:
        V_normal = P.CANInfo.Vnormal
        T_stable = P.CANInfo.Tstable_s
        T_count = P.CANInfo.Tcount  
        can_channel = DEFAULT_CAN_CHANNELS[0] if DEFAULT_CAN_CHANNELS else 1

        TestLog("INFO", "Step1",
                f"设置DUT电源电压为{V_normal}V，将KL30上电后，立即执行KL30下电，重复循环执行{T_count}次")
        kl30_power_cycle(T_count, V_normal, interval_s=0.1, step_name="Step1")

        TestLog("INFO", "Step2",
                f"设置DUT电源电压为{V_normal}V，将KL30上电，根据DUT通信唤醒方式，使用KL15上电或网络管理报文的方式唤醒CAN网络，等待{T_stable}s时间至通信稳定")
        ret = can_power_setup_and_communication_check(V_normal, T_stable)
        if ret != 0:
            TestLog("FAIL", "Step2", "电源设置与通信检查失败，结束测试")
            return

        ctx.can.set_filter_by_channel(can_channel)
        if not check_bus_error_frames("Step2"):
            return

        TestLog("PASS", "Step2", "多次冷启动快速上下电后，DUT的基础通信正常")

    except Exception as e:
        TestLog("FAIL", "冷启动快速上下电测试", f"测试执行出错: {e}")
        import traceback
        TestLog("DEBUG", "冷启动快速上下电测试", f"详细错误: {traceback.format_exc()}")

def test_TG2_TC18_SleepWakeupCycleTest():
    """
    休眠唤醒循环测试
    """
    try:
        ...
        T_stable = P.CANInfo.Tstable_s
        T_count = P.CANInfo.Tcount  
        T_periodDeviationDelay = P.CANInfo.TperiodDeviationDelay_min  
        rTperiodDeviation1 = P.CANInfo.TperiodDeviation1_pct
        rTperiodDeviation2 = P.CANInfo.TperiodDeviation2_pct
        can_channel = DEFAULT_CAN_CHANNELS[0] if DEFAULT_CAN_CHANNELS else 1
        ...
        for i in range(1, T_count + 1):
            TestLog("INFO", "Step1",
                    f"第{i}/{T_count}次 - 设置DUT电源电压为{V_normal}V，将KL30上电，唤醒CAN网络")

            ret = can_power_setup_and_communication_check(V_normal, T_stable)
            if ret != 0:
                TestLog("FAIL", "休眠唤醒循环测试", f"第{i}次唤醒失败，结束测试")
                return

            if not check_bus_error_frames(f"Step1-第{i}次"):
                return

            TestLog("INFO", "Step2", f"第{i}/{T_count}次 - 释放CAN网络，使DUT进入休眠")
            WakeupStop()
            sl_time().sleep(5 * 1000)

        TestLog("INFO", "Step3",
                f"设置DUT电源电压为{V_normal}V，将KL30上电，唤醒CAN网络")
        ret = can_power_setup_and_communication_check(V_normal, T_stable)
        if ret != 0:
            TestLog("FAIL", "Step3", "最终唤醒失败，结束测试")
            return

        if not check_bus_error_frames("Step3"):
            return

        TestLog("INFO", "Step4",
                f"持续监控总线{T_periodDeviationDelay}分钟，记录在这期间DUT发送的所有报文ID及对应的最大周期")

        ctx.can.clear_messages()
        ctx.can.set_filter_by_channel(can_channel)

        time.sleep(T_periodDeviationDelay * 60)

        check_bus_error_frames("Step4")

        messages = ctx.can.messages
        rx_stats = build_rx_msg_info(messages)
        TestLog("INFO", "Step4", f"监控完成，共接收到 {len(rx_stats)} 种不同ID的报文")
        ...
        TestLog("PASS", "", "多次休眠唤醒后，DUT的基础通信正常")

    except Exception as e:
        TestLog("FAIL", "休眠唤醒循环测试", f"测试执行出错: {e}")
        import traceback
        TestLog("DEBUG", "休眠唤醒循环测试", f"详细错误: {traceback.format_exc()}")

def test_TG2_TC19_RapidPowerCycleDiagnosticTest():
    """
    快速上下电后诊断功能测试
    """
    try:
        from testcases_canlin.can.can_diag_utils import service_10_check, get_can_node
        ...
        ret = can_power_setup_and_communication_check(V_normal, T_stable)
        if ret != 0:
            TestLog("FAIL", "快速上下电后诊断功能测试", "电源设置与通信检查失败，结束测试")
            return
        ...
        TestLog("INFO", "Step3", "发送诊断指令$10 01 和 $10 03")

        result_01 = service_10_check(
            node,
            session=0x01,
            expect_data=[0x50, 0x01],
            expect_str="$10 01 肯定响应(50 01)"
        )

        result_03 = service_10_check(
            node,
            session=0x03,
            expect_data=[0x50, 0x03],
            expect_str="$10 03 肯定响应(50 03)"
        )

        if result_01 and result_03:
            TestLog("PASS", "Step3", "期望结果：DUT能正常给出肯定响应应答。实际结果：DUT正常响应")
            TestLog("PASS", "快速上下电后诊断功能测试", "多次快速上下电后，DUT的基础诊断功能正常")
        else:
            TestLog("FAIL", "Step3", "期望结果：DUT能正常给出肯定响应应答。实际结果：DUT响应异常")

    except Exception as e:
        TestLog("FAIL", "快速上下电后诊断功能测试", f"测试执行出错: {e}")
        import traceback
        TestLog("DEBUG", "快速上下电后诊断功能测试", f"详细错误: {traceback.format_exc()}")

def test_TG2_TC20_VoltageBoundaryValueCommunicationTest():
    """
    电压边界值通信测试
    """
    try:
        ...
        ret = can_power_setup_and_communication_check(V_normal, T_stable)
        if ret != 0:
            TestLog("FAIL", "电压边界值通信测试", "电源设置与通信检查失败，结束测试")
            return
        ...
        for i in range(1, T_count + 1):
            TestLog("INFO", "", f"开始第{i}/{T_count}轮电压边界值测试")

            TestLog("INFO", "Step2", f"设置DUT的供电电压到{V_low_stand}V，持续监控总线{T_vStepDelay}ms")
            ...
            TestLog("INFO", "Step3", "记录在这期间DUT发送的所有报文ID及对应的最大周期")

            if (ctx.can.get_info('gErrorFrameCount') or 0) > 0:
                TestLog("FAIL", "Step3", f"第{i}轮低压测试 - 期望结果：总线无错误帧。实际结果：存在错误帧")
            ...
            TestLog("INFO", "Step4", f"设置DUT的供电电压到{V_high_stand}V，持续监控总线{T_vStepDelay}ms")

            ctx.can.clear_messages()
            ctx.power_ctrl.set_voltage(V_high_stand)
            sl_time().sleep(T_vStepDelay)

            TestLog("INFO", "Step5", "记录在这期间DUT发送的所有报文ID及对应的最大周期")

            if (ctx.can.get_info('gErrorFrameCount') or 0) > 0:
                TestLog("FAIL", "Step5", f"第{i}轮高压测试 - 期望结果：总线无错误帧。实际结果：存在错误帧")
                overall_result = False
            else:
                messages = ctx.can.messages
                rx_stats = build_rx_msg_info(messages)
        ...
        if overall_result:
            TestLog("PASS", "", "在低压和高压通信的边界值电压供电时，DUT的基础通信正常")
        else:
            TestLog("FAIL", "", "在低压和高压通信的边界值电压供电时，DUT的基础通信存在异常")

    except Exception as e:
        TestLog("FAIL", "电压边界值通信测试", f"测试执行出错: {e}")
        import traceback
        TestLog("DEBUG", "电压边界值通信测试", f"详细错误: {traceback.format_exc()}")

def test_TG2_TC21_WakeupTimeRecordTest():
    """
    唤醒时间记录测试
    """
    try:
        V_normal = P.CANInfo.Vnormal
        T_wakeupMax = P.CANInfo.TwakeupMax_ms
        can_channel = DEFAULT_CAN_CHANNELS[0] if DEFAULT_CAN_CHANNELS else 1

        nm_msg_id = P.ECUInfo.NMMsgID_int
        if nm_msg_id == 0:
            TestLog("FAIL", "", "未配置DUT网络管理报文ID，无法执行测试")
            return

        dut_force_sleep(wait_time_s=5)

        TestLog("INFO", "Step1",
                f"设置DUT电源电压为{V_normal}V，将KL30上电，根据DUT通信唤醒方式，使用KL15上电或网络管理报文的方式唤醒CAN网络，记录唤醒时刻T1")
        ...
        T1 = time.time() * 1000
        TestLog("INFO", "Step1", f"KL30上电时刻T1: {T1:.2f}ms")

        WakeupStart()
        T1_wakeup = sl_time().timestamp() * 1000
        # TestLog("INFO", "Step1", f"发送唤醒信号时刻: {T1_wakeup:.2f}ms")
        TestLog("INFO", "Step1", f"发送唤醒信号时刻: {T1_wakeup:.2f}ms")

        TestLog("INFO", "Step2", f"验证唤醒后收到的第一帧DUT发送的网络管理报文(0x{nm_msg_id:X})的时间为T2")

        T2 = None
        timeout_ms = T_wakeupMax * 2
        start_wait = time.time() * 1000

        while (time.time() * 1000 - start_wait) < timeout_ms:
            messages = ctx.can.messages
            for msg in messages:
                if hasattr(msg, 'id') and hasattr(msg, 'timestamp'):
                    if msg.id == nm_msg_id:
                        T2 = msg.timestamp * 1000 
                        break
            if T2 is not None:
                break
            time.sleep(0.01)
        ...
        if T2 is None:
            TestLog("FAIL", "Step2", f"在{timeout_ms}ms内未收到DUT发送的网络管理报文(0x{nm_msg_id:X})")
            return

        wakeup_time = T2 - T1_wakeup
        TestLog("INFO", "Step2", f"收到NM报文(0x{nm_msg_id:X}), T2={T2:.2f}ms, 唤醒时间={wakeup_time:.2f}ms")

        if wakeup_time <= T_wakeupMax:
            TestLog("PASS", "Step3", f"唤醒时间{wakeup_time:.2f}ms <= {T_wakeupMax}ms，满足要求")
        else:
            TestLog("WARNING", "Step3", f"唤醒时间{wakeup_time:.2f}ms > {T_wakeupMax}ms，超过最大唤醒时间")

    except Exception as e:
        TestLog("FAIL", "唤醒时间记录测试", f"测试执行出错: {e}")
        import traceback
        TestLog("DEBUG", "唤醒时间记录测试", f"详细错误: {traceback.format_exc()}")
...
def test_TG3_TC11_RealVehicleLoadStressTest():
    ...
    try:
        ...
        from testcases_canlin.can.can_diag_utils import CANBusSim
...
def test_TG5_TC2_DiagnosticServiceDuringCommunication():
    """
    通信中诊断服务测试
    """
    try:
        from testcases_canlin.can.can_diag_utils import get_can_node, send_diagnostic_sequence
...
def test_TG5_TC3_DiagnosticCommunicationTimeoutTest():
    """
    诊断通信超时测试
    """
    try:
        from testcases_canlin.can.can_diag_utils import service_19_check, get_can_node, get_dtc_list_from_19_resp
        ...
        T1 = None
        start_time = sl_time().tim2estamp()
        found_nm = False  # 标志变量
        while sl_time().timestamp() - start_time < 3:
            messages = ctx.can.messages
            for msg in messages:
                if msg.id == nm_msg_id:
                    T1 = msg.time_ms
...
def test_TG5_TC4_DiagnosticLowVoltageRangeTest():
    """
    诊断低压范围测试
    """
    try:
        from testcases_canlin.can.can_diag_utils import service_19_check, get_can_node, service_14_check, get_dtc_list_from_19_resp
...
def test_TG5_TC5_DiagnosticHighVoltageRangeTest():
    """
    诊断高压范围测试
    """
    try:
...
def test_TG5_TC6_DiagnosticLowVoltageRecoveryTimeTest():
    """
    诊断低压恢复时间测试
    """
    try:
...
def test_TG5_TC7_DiagnosticHighVoltageRecoveryTimeTest():
    """
    诊断高压恢复时间测试
    """
    try:
