    ...
    try:
        ...
        if dut_mode == "slave":
            ...
            try:
                ...
                TestLog("WARNING", "", f"切换失败，继续默认表: {e}")
...
def lin_node_power_setup_and_communication_check(vnormal, tstable_s):
    try:
        ...
        if get_test_case_mode() == "slave":
            if ActivateDut(0, tstable_s, vnormal) != 0:
                TestLog("FAIL", "", "DUT激活失败，结束测试")
                return -1
        else:
            TestLog("FAIL", "", "DUT激活失败，结束测试")
            return -1
        ...
        sch = create_lin_sch()
        sch.stop()
        TestLog("DEBUG", "", "LIN电源设置和通信检查成功")
        return 0
    except Exception as e:
        TestLog("ERROR", "", f"LIN电源设置和通信检查失败: {e}")
        return -1