...
def test_TG1_TC1_PhyLayer_LowVoltageRangeTest():
    """
    LIN物理层低电压范围测试
    """
    # 测试参数
...
def test_TG1_TC2_PhyLayer_HighVoltageRangeTest():
    """
    LIN物理层高电压范围测试
    """
    # 测试参数
...
def test_TG1_TC3_PhyLayer_DominantVlotage_Test():
    """
    显性电压测试,需要示波器
    """
    try:
...
def test_TG1_TC4_PhyLayer_RecessiveVlotage_Test():
    """
    Lin隐性电压测试,需要示波器
    """
    try:
...
def test_TG1_TC5_PhyLayer_GndOffset_Test():
    """
    地漂电源偏移测试
    """
    try:
...
def test_TG1_TC6_PhyLayer_BitTime_Test():
    """
    位时间测试
    """
    try:
...
def test_TG1_TC7_PhyLayer_Slope_Test():
    """
    斜率测试
    """
    try:
...
def test_TG1_TC8_PhyLayer_DutyRatio_Test():
    """
    占空比测试
    """
    try:
...
def test_TG1_TC9_PhyLayer_Resistance_Test():
    """
    电阻测试
    """
    try:
...
def test_TG2_TC5_ComLayer_BaudrateCompatibility_Test():
    """
    波特率兼容测试
    """
    # 测试参数配置
...
def test_TG2_TC6_ComLayer_FrameChecksum_Test():
    """
    帧校验方式测试
    """
    # 测试参数配置
...
def test_TG2_TC7_ComLayer_BreakField_DominantVlotageLength_Test():
    """
    同步间隔场显性电平长度测试
    """
    try:
...
def test_TG2_TC8_ComLayer_SyncField_Length_Test():
    """
    同步界定符电平长度测试
    """
    try:
...
def test_TG3_TC1_NMLayer_Sleep_Test():
    """
    网络休眠测试
    """
    rTdefaultWait = P.LINInfo.TdefaultWait_s  # 默认等待时间
    ...
    try:
        ...
        if get_test_case_mode()=="master":
            ...
            from .lin_module import send_message,create_lin_sch
            monitor_lin_communication(2)
            sch = create_lin_sch()
            sch.stop()
            ...
            TestLog("INFO", "网络休眠测试", f"唤醒总线，停止调度表")
            send_wakeup()
            sch = create_lin_sch()
            sch.stop()
            time.sleep(2)
...
def test_TG3_TC3_NMLayer_StartUpTime_Test():
    """
    初始化时间测试
    """
    rSimulationActivate = P.LINInfo.SimulationActivate  # 仿真激活标志
...
def test_TG4_TC6_FauLayer_CheckSumError_Test():
    """
    CheckSum场错误测试
    """
    try:
        ...
        TestLog("FAIL", "CheckSum场错误测试", f"测试执行出错: {e}")
        import traceback
        TestLog("DEBUG", "CheckSum场错误测试", f"详细错误: {traceback.format_exc()}")


def test_TG4_TC7_FauLayer_DataFieldError_Test():
    """
    数据场错误测试
    """
    try:
...
def test_TG4_TC8_FauLayer_IncomFrameDisturb_Test():
    """
    不完整帧干扰测试
    """
    try:
...
def test_TG4_TC9_FauLayer_ResponseError_Test():
    """
    应答错误测试
    """
    try:
        ...
        TestLog("FAIL", "应答错误测试", f"测试执行出错: {e}")
        import traceback
        TestLog("DEBUG", "应答错误测试", f"详细错误: {traceback.format_exc()}")
